...
const AdminLogin = () => {
  ...
  useEffect(() => {
    // If token exists, verify it and auto-redirect to dashboard
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--color-bg-base)] px-4">
      <div className="w-full max-w-md card p-8 md:p-10 shadow-[0_0_50px_rgba(16,185,129,0.05)] border-emerald-500/10">
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--color-text-secondary)] text-sm mt-2">Sign in to manage your portfolio</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="e.g., abdullah or admin@example.com"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-500/50 text-white rounded-lg font-semibold transition-all flex items-center justify-center"
          >
            {loading ? (
        ...
        <div className="mt-6 text-center text-sm text-[var(--color-text-secondary)]">
          <Link to="/" className="inline-flex items-center gap-1 mt-2 text-xs hover:text-white transition-colors">
            ← Back to Portfolio
          </Link>
        </div>
      </div>
